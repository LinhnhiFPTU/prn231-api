...
{
    public static IOrderedQueryable<T> OrderByMultipleFields<T>(this IQueryable<T> query,
        List<(string field, bool isDescending)> orderByFields)
    {
    ...
    private static IOrderedQueryable<T> ApplyThenOrdering<T>(IOrderedQueryable<T> query, string orderByField,
        bool isDescending)
    {
        var parameter = Expression.Parameter(typeof(T), "p");
        var property = Expression.Property(parameter, orderByField);
